...
— это интегрированная среда разработки на языке Python, созданная с помощью библиотеки Tkinter. Поставляется вместе с Python и благодаря использованию Tkinter может использоваться на многих платформах, среди которых Windows, Mac OS, Unix-подобные http://python.org/idle — официальный сайт IDLE
...
Делается это, чтобы вместо необходимости писать всякий раз gettext.gettext("Translate me") можно было писать
    _ ("Translate me").
(Чтобы можно было выполнить такой вызов, мы сначала должны импортировать модуль gettext, чтобы получить доступ к функции gettext (), находящейся в этом модуле).
...
Убедитесь, что Вы запустили PyChecker с вашим кодом. Для того чтобы узнать как запустить PyChecker, загляните на его домашнюю страницу - http://pychecker.sourceforge.net/. Чтобы подавить предупреждения, вам нужно создать переменную __pychecker__ в данном модуле и указать какие ошибки должны подавляться. Например:
...
flake8
====
covers both pyflakes and pep8. Strongly suggest it over just using one or the other.
https://flake8.readthedocs.io/en/2.0/
...
где v0 — начальная скорость мячика, g — ускорение свободного падения и t — время. Заметим, ось y выбрана так, что мячик в начальный момент времени (t = 0) находится в точке с координатой y = 0. Чтобы посмотреть сколько времени займет у мячика вернуться в эту же координату (y = 0), надо с одной стороны в выражении  правую часть приравнять нулю, с другой — вынести переменную t:

        t = 0 , t = 2*v0/g

Таким образом, мы определили, что возможное время полета мяча задается интервалом времени t2 [ 0 , 2*v0/g ]
...
Программа также более безопасна относительно наших исправлений, потому что мы видим к чему относится изменяемое нами число. К тому же, нам достаточно изменить переменную (например, t) всего лишь в одном месте, в остальных местах, где она появится она будет ссылаться на введенное нами значение.
...
наиболее часто встречающиеся методы форматирования:
------------------------------------------------------------------
     %s     строка
     %d     целое число
