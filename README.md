...
## Mac OS X
Щелкните на ссылке Mac OS X 64-bit/32-bit installer , чтобы загрузить файл с расширением .dmg для Mac. После завершения загрузки дважды щелкните на нем. Появится окно с четырьмя значками. Правой кнопкой мыши щелкните на Python.mpkg и затем в появившемся диалоговом окне нажмите кнопку Open (Открыть). Нажмите кнопку Continue (Продолжить) три раза, чтобы просмот­реть юридические детали, и затем, когда появится соответствующее диалоговое окно, нажмите кнопку Install (Установить). Python 3 будет установлен в каталог /usr/local/bin/python3 , что оставит существующую версию Python 2 нетронутой.
